...
vars_net3 <- vars_net %>%  # Start with the summarized variables data
...
vars_net5 <- vars_net %>%  # Start with the summarized variables data
    mutate(Abandoned_Cars.nn = nn_function(  # Create a new column for nearest neighbor information
        st_c(st_coid(vars_net)),  # Calculate centroids of fishnet grid cells
        st_c(abandonCars),         # Get coordinates of abandoned cars
...
ggplot() +
      geom_sf(data = vars_net3.long.nn, aes(fill=value), colour=NA) +
      scale_fill_viridis(name="NN3 Distance") +
      labs(title="Abandoned Car NN3 Distance") +
      theme_void()
